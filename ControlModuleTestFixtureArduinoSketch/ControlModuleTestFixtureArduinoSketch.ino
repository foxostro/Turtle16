...
void testJmp(unsigned ledState) {
  ...
  assertEqual(0b111111011101010111111, actualTestFixtureInputs.Ctl_EX, "Expect appropriate control signals for JMP");
  
  printf("passed\n");
}

void testStallOnStoreOp(unsigned ledState) {
  TestFixtureOutputs testFixtureOutputs;

  printf("%s: ", __FUNCTION__);
  printf("Stall due to dependency on a LOAD instruction...");
  ...
  TestFixtureInputs actualTestFixtureInputs = testFixtureInputPorts.read();
  assertEqual(1, actualTestFixtureInputs.stall, "Expect stall==1");
