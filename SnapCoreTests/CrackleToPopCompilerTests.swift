//
//  CrackleToPopCompilerTests.swift
//  SnapCoreTests
//
...
class CrackleToPopCompilerTests: XCTestCase {
    ...
    func mustCompile(_ crackleInstructions: [CrackleInstruction]) -> [Instruction] {
        return try! compile(crackle: crackleInstructions)
    }
    
    func compile(crackle crackleInstructions: [CrackleInstruction]) throws -> [Instruction] {
        let compiler = CrackleToPopCompiler()
        try compiler.compile(ir: crackleInstructions)
        let machineLanguageInstructions = try compile(pop: compiler.instructions)
        return machineLanguageInstructions
    }
    
    func compile(pop: [PopInstruction]) throws -> [Instruction] {
        let microcodeGenerator = MicrocodeGenerator()
        microcodeGenerator.generate()
        let assembler = AssemblerBackEnd(microcodeGenerator: microcodeGenerator)
        let popCompiler = PopCompiler(assembler: assembler)
        try popCompiler.compile(pop: pop)
        let machineLanguageInstructions = popCompiler.instructions
        return machineLanguageInstructions
    }
    ...
    func testCompileFailsBecauseLabelRedefinesExistingLabel() {
        ...
        XCTAssertThrowsError(try compile(crackle: instructions)) {
