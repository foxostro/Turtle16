...
class SnapLexerTests: XCTestCase {
    ...
    func testTokenizeImpl() {
        let tokenizer = SnapLexer(withString: "impl")
        tokenizer.scanTokens()
    ...
    func testTokenizeTypealias() {
        let tokenizer = SnapLexer(withString: "typealias")
        tokenizer.scanTokens()
        XCTAssertEqual(tokenizer.tokens, [TokenTypealias(sourceAnchor: tokenizer.lineMapper.anchor(0, 9)),
                                          TokenEOF(sourceAnchor: tokenizer.lineMapper.anchor(9, 9))])
    }
    
    func testTokenizeMatch() {
        let tokenizer = SnapLexer(withString: "match")
        tokenizer.scanTokens()
    ...
    func testTokenizeAssert() {
        let tokenizer = SnapLexer(withString: "assert")
        tokenizer.scanTokens()
        XCTAssertEqual(tokenizer.tokens, [TokenAssert(sourceAnchor: tokenizer.lineMapper.anchor(0, 6)),
                                          TokenEOF(sourceAnchor: tokenizer.lineMapper.anchor(6, 6))])
    }
    
    func testTokenizeIdentifierWithLeadingUnderscores() {
        let tokenizer = SnapLexer(withString: "__testMain")
        tokenizer.scanTokens()
        XCTAssertEqual(tokenizer.tokens, [TokenIdentifier(sourceAnchor: tokenizer.lineMapper.anchor(0, 10)),
                                          TokenEOF(sourceAnchor: tokenizer.lineMapper.anchor(10, 10))])
    }
    ...
    func testTokenizeTest() {
        let tokenizer = SnapLexer(withString: "test")
        tokenizer.scanTokens()
        XCTAssertEqual(tokenizer.tokens, [TokenTest(sourceAnchor: tokenizer.lineMapper.anchor(0, 4)),
                                          TokenEOF(sourceAnchor: tokenizer.lineMapper.anchor(4, 4))])
    }
}
