...
public final class CompilerPassDecomposeExpressions: CompilerPassWithDeclScan {
    ...
    public override func visit(
        ...
        return try node.withExpression(
            extract(expr: visit(expr: e))
        )
    }

    public override func visit(if node: If) throws -> AbstractSyntaxTreeNode? {
        try node.withCondition(
            extract(expr: visit(expr: node.condition))!
        )
    }
    ...
    public override func visit(match node: Match) throws -> AbstractSyntaxTreeNode? {
        try node.withExpr(
            extract(expr: visit(expr: node.expr))!
        )
    }
    ...
    public override func visit(as node0: As) throws -> Expression? {
        ...
        let expr1: Expression! = try extract(expr: visit(expr: expr0))
    ...
    public override func visit(bitcast node0: Bitcast) throws -> Expression? {
        try node0.withExpr(
            extract(expr: visit(expr: node0.expr))!
        )
    }
    ...
    public override func visit(binary node0: Binary) throws -> Expression? {
        try node0
            .withLeft(
                extract(expr: visit(expr: node0.left))!
            )
            .withRight(
                extract(expr: visit(expr: node0.right))!
            )
    }
    ...
    public override func visit(assignment node0: Assignment) throws -> Expression? {
        ...
        default:
            let lexpr = try lextract(expr: visit(expr: node0.lexpr))!
            let rexpr = try extract(expr: visit(expr: node0.rexpr))!
            let node1 = node0
                .withLexpr(lexpr)
                .withRexpr(rexpr)
            return node1
    ...
    public override func visit(subscript node0: Subscript) throws -> Expression? {
        ...
        let argument =
            ...
            ident.identifier == "Range" {
                ...
                try extract(expr: visit(expr: node0.argument))
            }
        let subscriptable =
            ...
            default:
                try extract(
                    expr: visit(expr: node0.subscriptable)
                )
            }
        let node1 = node0
    ...
    public override func visit(structInitializer node0: StructInitializer) throws -> Expression? {
        ...
        if node0.arguments.count > 0 {
            ...
            let backingStoragePointerTypeExpr = try ConstType(
                rvalueContext
    ...
    public override func visit(call node0: Call) throws -> Expression? {
        try node0
            .withArguments(
                node0.arguments.map { arg in
                    try extract(expr: visit(expr: arg))!
                }
            )
            .withCallee(
                {
    ...
    private func isCompilerTemporary(_ expr: Expression?) -> Bool {
        ...
        return (type?.isPrimitive == true) && expr.identifier.hasPrefix("__")
    }
}

public extension AbstractSyntaxTreeNode {
    /// Decompose expressions into simpler ones connected by temporary values
    func decomposeExpressions() throws -> AbstractSyntaxTreeNode? {
