...
public final class CompilerPassGenerics: CompilerPassWithDeclScan {
    ...
    fileprivate typealias PendingInsertions = [(
        AbstractSyntaxTreeNode.ID, [AbstractSyntaxTreeNode]
    )]
    fileprivate var pendingInsertions: PendingInsertions = []

    fileprivate func appendPendingInsertion(
        _ node: AbstractSyntaxTreeNode,
        after id: AbstractSyntaxTreeNode.ID
    ) {
        appendPendingInsertions([node], at: id)
    }

    fileprivate func appendPendingInsertions(
        _ nodes: [AbstractSyntaxTreeNode],
        at id: AbstractSyntaxTreeNode.ID
    ) {
