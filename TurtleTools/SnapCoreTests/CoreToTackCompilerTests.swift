...
final class CoreToTackCompilerTests: XCTestCase {
    ...
    func testRvalue_As_union_to_non_primitive() throws {
        let symbols = Env(tuples: [
            (
                "foo",
                Symbol(
                    type: .unionType(UnionTypeInfo([.array(count: 1, elementType: .u16)])),
                    offset: 0xabcd,
                    qualifier: .staticStorage
                )
            )
    ...
    func testRvalue_As_convert_primitive_value_to_union() throws {
        ...
        let compiler = makeCompiler(symbols: symbols)
        let actual = try compiler.rvalue(
            expr: As(expr: Identifier("foo"), targetType: UnionType([PrimitiveType(.u16)]))
        )
        let expected = Seq(children: [
    ...
    func testRvalue_As_determine_union_type_tag() throws {
        ...
        let actual = try compiler.rvalue(
            expr: As(
                expr: Identifier("foo"),
                targetType: UnionType([PrimitiveType(.bool), PrimitiveType(.u16)])
            )
        )
        let expected = Seq(children: [
            TackInstructionNode(.subip(.p(0), .fp, 2)),
        ...
        let actual = try compiler.rvalue(
            expr: As(
                ...
                targetType: UnionType([
                    ArrayType(count: LiteralInt(2), elementType: PrimitiveType(.u16))
                ])
            )
        )
        let expected = Seq(children: [
    ...
    func testRvalue_As_union_to_primitive_with_dynamic_type_check() throws {
        let symbols = Env(tuples: [
            (
                "foo",
                Symbol(
                    type: .unionType(UnionTypeInfo([.bool, .u16])),
                    offset: 0xabcd,
                    qualifier: .staticStorage
                )
            )
    ...
    func testRvalue_Is_test_union_type_tag() throws {
        ...
        let compiler = makeCompiler(symbols: symbols)
        let actual = try compiler.rvalue(
            expr: Is(expr: Identifier("foo"), testType: PrimitiveType(.bool))
        )
        let expected = Seq(children: [
    ...
    func testRvalue_Is_test_union_type_tag_const() throws {
        let symbols = Env(tuples: [
            (
                ...
                Symbol(
                    type: .unionType(
                        UnionTypeInfo([.arithmeticType(.immutableInt(.u8)), .constBool])
                    ),
                    offset: 100,
                    qualifier: .staticStorage
                )
    ...
    func testRvalue_Assignment_automatic_conversion_from_trait_to_pointer() throws {
        ...
        symbols.bind(
            identifier: "bar",
            symbol: Symbol(type: traitType, offset: 0x2000, qualifier: .staticStorage)
        )
                ...
                Symbol(
                    type: .function(
                        FunctionTypeInfo(
                            ...
                            mangledName: "foo",
                            returnType: .u16,
                            arguments: [.unionType(UnionTypeInfo([.u16]))]
                        )
                    )
                )
            ),
    ...
    func testRvalue_Assignment_with_StructInitializer() throws {
        ...
        symbols.bind(
            identifier: "foo",
            symbol: Symbol(type: kSliceType, offset: 0x1000, qualifier: .staticStorage)
        )
        symbols.bind(identifier: kSliceName, symbolType: kSliceType)
        let compiler = makeCompiler(symbols: symbols)
        ...
        let symbols = Env(
            tuples: [
                (
                    "none",
                    Symbol(
                        type: None.correspondingConstType,
