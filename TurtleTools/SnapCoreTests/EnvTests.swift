...
final class EnvTests: XCTestCase {
    ...
    func testUseOfUnresolvedIdentifier() {
        ...
        XCTAssertThrowsError(
            try symbols.resolveWithStackFrameDepth(sourceAnchor: nil, identifier: "foo")
        ) {
            let error = $0 as? CompilerError
            XCTAssertNotNil(error)
            XCTAssertEqual(error?.message, "use of unresolved identifier: `foo'")
    ...
    func testBindWord_Static_Constant() {
        ...
        symbols.bind(
            identifier: "foo",
            symbol: Symbol(type: .arithmeticType(.immutableInt(.u8)), offset: 0x10)
        )
        let symbol = try! symbols.resolve(sourceAnchor: nil, identifier: "foo")
    ...
    func testSuccessfullyResolveTypeByIdentifier() {
        let symbols = Env(
            parent: nil,
            typeDict: ["foo": .structType(StructTypeInfo(name: "foo", fields: Env()))]
        )
    ...
    func testBindStructType() {
        ...
        symbols.bind(
            identifier: "foo",
            symbolType: .structType(StructTypeInfo(name: "foo", fields: Env()))
        )
        let symbolType = try! symbols.resolveType(identifier: "foo")
        switch symbolType {
        case .structType(let typ):
