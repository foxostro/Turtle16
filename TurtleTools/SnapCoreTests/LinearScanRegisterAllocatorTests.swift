...
final class LinearScanRegisterAllocatorTests: XCTestCase {
    ...
    func testDoNotModifyRangesWithExistingAllocations1() throws {
        let actual = LinearScanRegisterAllocator.allocate(
            numRegisters: 4,
            liveIntervals: [
                LiveInterval(
                    range: 0..<Int.max,
                    virtualRegisterName: "vr0",
                    physicalRegisterName: "r5"
                )
            ]
        )
        XCTAssertEqual(
