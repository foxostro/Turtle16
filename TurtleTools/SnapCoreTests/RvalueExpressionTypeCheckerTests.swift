...
extension SymbolTable {
    func withCompilerIntrinsicRangeType(_ memoryLayoutStrategy: MemoryLayoutStrategy) -> SymbolTable {
        ...
        let name = "Range"
        let typ: SymbolType = .structType(StructTypeInfo(name: name, symbols: SymbolTable(tuples: [
...
final class RvalueExpressionTypeCheckerTests: XCTestCase {
    ...
    func testUnsupportedExpressionThrows() {
        ...
        XCTAssertThrowsError(try typeChecker.check(expression: UnsupportedExpression(sourceAnchor: nil))) {
    ...
    func testBinary_IntegerConstant_Eq_IntegerConstant() {
        let typeChecker = RvalueExpressionTypeChecker()
        let expr = ExprUtils.makeComparisonEq(left: LiteralInt(1000),
    ...
    func testBinary_BooleanConstant_Eq_Bool() {
        let typeChecker = RvalueExpressionTypeChecker()
        let expr = ExprUtils.makeComparisonEq(left: LiteralBool(false),
                                              right: ExprUtils.makeBool(value: false))
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
        XCTAssertEqual(result, .bool)
    ...
    func testBinary_IntegerConstant_Ne_IntegerConstant() {
        let typeChecker = RvalueExpressionTypeChecker()
        let expr = ExprUtils.makeComparisonNe(left: LiteralInt(1000),
                                              right: LiteralInt(1000))
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
    ...
    func testBinary_BooleanConstant_Ne_IntegerConstant() {
        let typeChecker = RvalueExpressionTypeChecker()
        let expr = ExprUtils.makeComparisonNe(left: LiteralBool(false),
                                              right: LiteralInt(1))
        XCTAssertThrowsError(try typeChecker.check(expression: expr)) {
            let compilerError = $0 as? CompilerError
            XCTAssertNotNil(compilerError)
    ...
    func testBinary_IntegerConstant_Lt_IntegerConstant() {
        let typeChecker = RvalueExpressionTypeChecker()
        let expr = ExprUtils.makeComparisonLt(left: LiteralInt(1000),
                                              right: LiteralInt(1000))
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
        XCTAssertEqual(result, .booleanType(.compTimeBool(false)))
    ...
    func testBinary_IntegerConstant_Gt_IntegerConstant() {
        let typeChecker = RvalueExpressionTypeChecker()
        let expr = ExprUtils.makeComparisonGt(left: LiteralInt(1000),
                                              right: LiteralInt(1000))
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
        XCTAssertEqual(result, .booleanType(.compTimeBool(false)))
    ...
    func testBinary_IntegerConstant_Le_IntegerConstant() {
        let typeChecker = RvalueExpressionTypeChecker()
        let expr = ExprUtils.makeComparisonLe(left: LiteralInt(1000),
                                              right: LiteralInt(1000))
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
        XCTAssertEqual(result, .booleanType(.compTimeBool(true)))
    ...
    func testBinary_U8_Le_U8() {
        let typeChecker = RvalueExpressionTypeChecker()
        let expr = ExprUtils.makeComparisonLe(left: LiteralInt(1),
                                              right: LiteralInt(1))
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
        XCTAssertEqual(result, .booleanType(.compTimeBool(true)))
    ...
    func testBinary_IntegerConstant_Ge_IntegerConstant() {
        let typeChecker = RvalueExpressionTypeChecker()
        let expr = ExprUtils.makeComparisonGe(left: LiteralInt(1000),
                                              right: LiteralInt(1000))
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
        XCTAssertEqual(result, .booleanType(.compTimeBool(true)))
   ...
   func testBinary_U16_Minus_IntegerConstant() {
    ...
    func testBinary_IntegerConstant_BitwiseAnd_IntegerConstant() {
        let typeChecker = RvalueExpressionTypeChecker()
        let expr = Binary(op: .ampersand,
                                     left: LiteralInt(0b1010101010101010),
                                     right: LiteralInt(0b1111111111111111))
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
    ...
    func testBinary_IntegerConstant_LeftShift_IntegerConstant() {
        let typeChecker = RvalueExpressionTypeChecker()
        let expr = Binary(op: .leftDoubleAngle,
                                     left: LiteralInt(1),
                                     right: LiteralInt(2))
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
    ...
    func testBinary_IntegerConstant_RightShift_IntegerConstant() {
        let typeChecker = RvalueExpressionTypeChecker()
        let expr = Binary(op: .rightDoubleAngle,
                                     left: LiteralInt(2),
                                     right: LiteralInt(1))
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
    ...
    func testBinary_IntegerConstant_LogicalAnd_IntegerConstant() {
        let typeChecker = RvalueExpressionTypeChecker()
        let expr = Binary(op: .doubleAmpersand,
                                     left: LiteralInt(2),
                                     right: LiteralInt(1))
        XCTAssertThrowsError(try typeChecker.check(expression: expr)) {
            let compilerError = $0 as? CompilerError
    ...
    func testFailBecauseFunctionCallUsesIncorrectParameterType() {
        let expr = Call(callee: Identifier("foo"),
                                   arguments: [ExprUtils.makeBool(value: false)])
        let symbols = SymbolTable(tuples: [
            ("foo", Symbol(type: .function(FunctionTypeInfo(name: "foo", returnType: .u8, arguments: [.u8])), offset: 0x0000))
        ])
        let typeChecker = RvalueExpressionTypeChecker(symbols: symbols)
        XCTAssertThrowsError(try typeChecker.check(expression: expr)) {
            let compilerError = $0 as? CompilerError
    ...
    func testBoolasVoid() {
        let expr = As(expr: ExprUtils.makeBool(value: false),
    ...
    func testU8asVoid() {
        let expr = As(expr: ExprUtils.makeU8(value: 1),
                                 targetType: PrimitiveType(.void))
        let typeChecker = RvalueExpressionTypeChecker()
        XCTAssertThrowsError(try typeChecker.check(expression: expr)) {
            let compilerError = $0 as? CompilerError
    ...
    func testU8asI16() {
        ...
        let expr = As(expr: ExprUtils.makeU8(value: 0xff),
                                 targetType: PrimitiveType(.i16))
        let typeChecker = RvalueExpressionTypeChecker()
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
    ...
    func testU16asVoid() {
        let expr = As(expr: ExprUtils.makeU16(value: 0xffff),
                                 targetType: PrimitiveType(.void))
        let typeChecker = RvalueExpressionTypeChecker()
        XCTAssertThrowsError(try typeChecker.check(expression: expr)) {
            let compilerError = $0 as? CompilerError
    ...
    func testI16asVoid() {
        let expr = As(expr: ExprUtils.makeI16(value: -1),
                                 targetType: PrimitiveType(.void))
        let typeChecker = RvalueExpressionTypeChecker()
        XCTAssertThrowsError(try typeChecker.check(expression: expr)) {
            let compilerError = $0 as? CompilerError
    ...
    func testCannotConvertArrayLiteralsOfDifferentLengths() {
        let expr = As(expr: LiteralArray(arrayType: ArrayType(count: LiteralInt(1), elementType: PrimitiveType(.u8)),
    ...
    func testIntegerConstantAsU8_Overflows() {
        let expr = As(expr: LiteralInt(256),
                                 targetType: PrimitiveType(.u8))
        let typeChecker = RvalueExpressionTypeChecker()
        XCTAssertThrowsError(try typeChecker.check(expression: expr)) {
            let compilerError = $0 as? CompilerError
    ...
    func testIntegerConstantAsU16_Overflows() {
        let expr = As(expr: LiteralInt(65536),
                                 targetType: PrimitiveType(.u16))
        let typeChecker = RvalueExpressionTypeChecker()
        XCTAssertThrowsError(try typeChecker.check(expression: expr)) {
            let compilerError = $0 as? CompilerError
    ...
    func testIntegerConstantAsI8_Overflows() {
        let expr = As(expr: LiteralInt(128),
                                 targetType: PrimitiveType(.i8))
        let typeChecker = RvalueExpressionTypeChecker()
        XCTAssertThrowsError(try typeChecker.check(expression: expr)) {
            let compilerError = $0 as? CompilerError
    ...
    func testIntegerConstantAsI16_Overflows() {
        let expr = As(expr: LiteralInt(32768),
                                 targetType: PrimitiveType(.i16))
        let typeChecker = RvalueExpressionTypeChecker()
        XCTAssertThrowsError(try typeChecker.check(expression: expr)) {
            let compilerError = $0 as? CompilerError
    ...
    func testIntegerConstantAsI8_Overflows_Negative() {
        let expr = As(expr: LiteralInt(-129),
                                 targetType: PrimitiveType(.i8))
        let typeChecker = RvalueExpressionTypeChecker()
        XCTAssertThrowsError(try typeChecker.check(expression: expr)) {
            let compilerError = $0 as? CompilerError
    ...
    func testIntegerConstantAsI16_Overflows_Negative() {
        let expr = As(expr: LiteralInt(-32769),
                                 targetType: PrimitiveType(.i16))
        let typeChecker = RvalueExpressionTypeChecker()
        XCTAssertThrowsError(try typeChecker.check(expression: expr)) {
            let compilerError = $0 as? CompilerError
    ...
    func testBooleanConstantasBool() {
        let expr = As(expr: LiteralBool(false),
                                 targetType: PrimitiveType(.bool))
        let typeChecker = RvalueExpressionTypeChecker()
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
    ...
    func testEmptyArray() {
        let typeChecker = RvalueExpressionTypeChecker()
        var result: SymbolType? = nil
        let arr = LiteralArray(arrayType: ArrayType(count: LiteralInt(0), elementType: PrimitiveType(.u8)),
                                          elements: [])
    ...
    func testSingletonArrayOfU8() {
        ...
        let arr = LiteralArray(arrayType: ArrayType(count: LiteralInt(1), elementType: PrimitiveType(.u8)),
                                          elements: [val])
        XCTAssertNoThrow(result = try typeChecker.check(expression: arr))
    ...
    func testSingletonArrayOfU16() {
        ...
        let arr = LiteralArray(arrayType: ArrayType(count: LiteralInt(1), elementType: PrimitiveType(.u16)),
                                          elements: [val])
        XCTAssertNoThrow(result = try typeChecker.check(expression: arr))
        XCTAssertEqual(result, .array(count: 1, elementType: .u16))
    }
    ...
    func testSingletonArrayOfBoolean() {
        ...
        let arr = LiteralArray(arrayType: ArrayType(count: LiteralInt(1), elementType: PrimitiveType(.bool)),
                                          elements: [val])
        XCTAssertNoThrow(result = try typeChecker.check(expression: arr))
    ...
    func testSingletonArrayOfArray() {
        let typeChecker = RvalueExpressionTypeChecker()
        var result: SymbolType? = nil
        let val = LiteralArray(arrayType: ArrayType(count: LiteralInt(0), elementType: PrimitiveType(.u8)),
                                          elements: [])
        let arr = LiteralArray(arrayType: ArrayType(count: LiteralInt(1), elementType: ArrayType(count: LiteralInt(0), elementType: PrimitiveType(.u8))),
                                          elements: [val])
        XCTAssertNoThrow(result = try typeChecker.check(expression: arr))
    ...
    func testArrayOfU8() {
        ...
        var result: SymbolType? = nil
        let val = ExprUtils.makeU8(value: 0)
        let arr = LiteralArray(arrayType: ArrayType(count: LiteralInt(2), elementType: PrimitiveType(.u8)),
                                          elements: [val, val])
        XCTAssertNoThrow(result = try typeChecker.check(expression: arr))
    ...
    func testArrayOfU16() {
        ...
        var result: SymbolType? = nil
        let val = ExprUtils.makeU16(value: 1000)
        let arr = LiteralArray(arrayType: ArrayType(count: LiteralInt(2), elementType: PrimitiveType(.u16)),
                                          elements: [val, val])
        XCTAssertNoThrow(result = try typeChecker.check(expression: arr))
    ...
    func testArrayOfI8() {
        ...
        let arr = LiteralArray(arrayType: ArrayType(count: LiteralInt(2), elementType: PrimitiveType(.i8)),
                                          elements: [val, val])
        XCTAssertNoThrow(result = try typeChecker.check(expression: arr))
    ...
    func testArrayOfI16() {
        ...
        let arr = LiteralArray(arrayType: ArrayType(count: LiteralInt(2), elementType: PrimitiveType(.i16)),
                                          elements: [val, val])
        XCTAssertNoThrow(result = try typeChecker.check(expression: arr))
    ...
    func testArrayOfBoolean() {
        ...
        var result: SymbolType? = nil
        let val = LiteralBool(false)
        let arr = LiteralArray(arrayType: ArrayType(count: LiteralInt(2), elementType: PrimitiveType(.bool)),
                                          elements: [val, val])
        XCTAssertNoThrow(result = try typeChecker.check(expression: arr))
    ...
    func testArrayOfArray() {
        ...
        let val = LiteralArray(arrayType: ArrayType(count: LiteralInt(0), elementType: PrimitiveType(.u8)),
                                          elements: [])
        let arr = LiteralArray(arrayType: ArrayType(count: LiteralInt(2), elementType: ArrayType(count: LiteralInt(0), elementType: PrimitiveType(.u8))),
                                          elements: [val, val])
        XCTAssertNoThrow(result = try typeChecker.check(expression: arr))
    ...
    func testArrayLiteralHasNonConvertibleType() {
        let expr = LiteralArray(arrayType: ArrayType(count: LiteralInt(2), elementType: PrimitiveType(.bool)),
    ...
    func testInferTypeOfArrayOfIntegerConstantsWhichFitIntoU8() {
        let typeChecker = RvalueExpressionTypeChecker()
        var result: SymbolType? = nil
        let arr = LiteralArray(arrayType: ArrayType(count: nil, elementType: PrimitiveType(.u8)),
                                          elements: [LiteralInt(0),
                                                     LiteralInt(1),
                                                     LiteralInt(2)])
        XCTAssertNoThrow(result = try typeChecker.check(expression: arr))
    ...
    func testInferTypeOfArrayOfIntegerConstantsWhichFitIntoU16() {
        let typeChecker = RvalueExpressionTypeChecker()
        var result: SymbolType? = nil
        let arr = LiteralArray(arrayType: ArrayType(count: nil, elementType: PrimitiveType(.u16)),
                                          elements: [LiteralInt(0),
                                                     LiteralInt(0),
                                                     LiteralInt(1000)])
        XCTAssertNoThrow(result = try typeChecker.check(expression: arr))
    ...
    func testInferTypeOfArrayOfHeterogeneousArithmeticTypesWhichFitIntoU8() {
        let expr = LiteralArray(arrayType: ArrayType(count: LiteralInt(3), elementType: PrimitiveType(.u8)),
    ...
    func testInferTypeOfArrayOfIntegerConstantsWhichFitIntoI8_1() {
        let typeChecker = RvalueExpressionTypeChecker()
        var result: SymbolType? = nil
        let arr = LiteralArray(arrayType: ArrayType(count: nil, elementType: PrimitiveType(.i8)),
    ...
    func testInferTypeOfArrayOfIntegerConstantsWhichFitIntoI16_1() {
        let typeChecker = RvalueExpressionTypeChecker()
        var result: SymbolType? = nil
        let arr = LiteralArray(arrayType: ArrayType(count: nil, elementType: PrimitiveType(.i16)),
                                          elements: [LiteralInt(-1000),
                                                     LiteralInt(-2000),
                                                     LiteralInt(-3000)])
        XCTAssertNoThrow(result = try typeChecker.check(expression: arr))
    ...
    func testCannotAssignFunctionToArray() {
        let symbols = SymbolTable(tuples: [
            ("foo", Symbol(type: .function(FunctionTypeInfo(returnType: .bool, arguments: [.u8, .u16])), offset: 0x0010)),
    ...
    func testCountOfArrayTypeIsConstIntExpressionAndWeCanDoMathThere() {
        let expr = ArrayType(count: Binary(op: .plus, left: LiteralInt(1), right: LiteralInt(1)), elementType: PrimitiveType(.u8))
        let typeChecker = RvalueExpressionTypeChecker()
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
        XCTAssertEqual(result, .array(count: 2, elementType: .u8))
    ...
    func testStructInitializerExpression_Empty() {
        let expr = StructInitializer(identifier: Identifier("Foo"), arguments: [])
        let typ: SymbolType = .structType(StructTypeInfo(name: "Foo", symbols: SymbolTable()))
    ...
    func testResolveUnionTypeExpression() {
        ...
        let expected: SymbolType = .unionType(UnionTypeInfo([
            .u8,
            .u16,
            .bool,
    ...
    func testTestPrimitiveTypeIsExpression_Succeeds() {
        let expr = Is(expr: ExprUtils.makeU8(value: 0),
    ...
    func testSubscriptAnArrayWithARange() {
        ...
        let range = StructInitializer(identifier: Identifier("Range"), arguments: [
    ...
    func testAddressOfFunctionEvaluatesToFunctionPointerType() {
        ...
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
        let expected: SymbolType = .pointer(.function(FunctionTypeInfo(returnType: .void, arguments: [])))
    ...
    func testCallFunctionThroughFunctionPointer() {
        ...
        let symbols = SymbolTable(tuples: [
            ("foo", Symbol(type: .function(FunctionTypeInfo(name: "foo", returnType: .void, arguments: [])), offset: 0)),
            ("bar", Symbol(type: .pointer(.function(FunctionTypeInfo(name: "foo", returnType: .void, arguments: []))), offset: addressOfBar)),
        ])
        let typeChecker = RvalueExpressionTypeChecker(symbols: symbols)
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
    ...
    func testBitcastBoolAsU8() {
        let expr = Bitcast(expr: ExprUtils.makeU8(value: 0),
                                      targetType: PrimitiveType(.bool))
        let typeChecker = RvalueExpressionTypeChecker()
        var result: SymbolType? = nil
        XCTAssertNoThrow(result = try typeChecker.check(expression: expr))
    ...
    func testInitialAssignment_automatic_conversion_from_struct_to_trait_object() throws {
        let traitObjectType: SymbolType = .traitType(TraitTypeInfo(name: "Foo", nameOfTraitObjectType: "", nameOfVtableType: "", symbols: SymbolTable()))
        let symbols = SymbolTable(tuples: [
            ("__Foo_Bar_vtable_instance", Symbol(type: .structType(StructTypeInfo(name: "", symbols: SymbolTable())))),
            ("bar", Symbol(type: .structType(StructTypeInfo(name: "Bar", symbols: SymbolTable())))),
            ("foo", Symbol(type: traitObjectType))
        ])
        
        let typeChecker = RvalueExpressionTypeChecker(symbols: symbols)
        let expr = InitialAssignment(lexpr: Identifier("foo"),
                                                rexpr: Identifier("bar"))
        let result = try typeChecker.check(expression: expr)
    ...
    func testFunctionType() throws {
        let expected = SymbolType.function(FunctionTypeInfo(name: "foo", mangledName: "foo", returnType: .void, arguments: [.arithmeticType(.immutableInt(.u16))], ast: nil))
        let typeChecker = RvalueExpressionTypeChecker()
        let expr = FunctionType(name: "foo",
    ...
    func testGenericFunctionApplication() throws {
        ...
        let expected = SymbolType.function(FunctionTypeInfo(name: "foo[const u16]",
    ...
    func testCannotTakeTheAddressOfGenericFunctionWithInappropriateTypeArguments() {
        ...
        let expr = Unary(op: .ampersand, expression: GenericTypeApplication(identifier: Identifier("foo"), arguments: [PrimitiveType(.constU16), PrimitiveType(.constU16)]))
        let typeChecker = RvalueExpressionTypeChecker(symbols: symbols)
        XCTAssertThrowsError(try typeChecker.check(expression: expr)) {
            let compilerError = $0 as? CompilerError
            XCTAssertNotNil(compilerError)
    ...
    func testGenericStructApplication_StructInitializer() throws {
        ...
        let typeChecker = RvalueExpressionTypeChecker(symbols: symbols)
        let app = GenericTypeApplication(identifier: Identifier("foo"),
                                                    arguments: [PrimitiveType(.u16)])
        let expr = StructInitializer(expr: app, arguments: [
    ...
    func testCannotInstantiateGenericTraitTypeWithoutApplication() throws {
        let template = TraitDeclaration(
            ...
            visibility: .privateVisibility)
        let symbols = SymbolTable()
        symbols.bind(identifier: "Foo", symbol: Symbol(type: .genericTraitType(GenericTraitTypeInfo(template: template))))
    ...
    func testGenericTraitApplication_OneMember() throws {
        ...
        let bar = Symbol(
            type: .pointer(.function(FunctionTypeInfo(
                name: "bar",
