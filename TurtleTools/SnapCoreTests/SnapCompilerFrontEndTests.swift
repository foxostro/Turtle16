...
final class SnapCompilerFrontEndTests: XCTestCase {
    ...
    func test_EndToEndIntegration_SubscriptSliceThroughPointer() throws {
        let debugger = try run(
            program: """
                let arr = [_]u16{0x1000}
                let slice: []u16 = arr
    ...
    func test_EndToEndIntegration_SubscriptSliceThroughPointer_WithRuntimeBoundsChecking() throws {
        let debugger = try run(
            options: Options(isBoundsCheckEnabled: true, runtimeSupport: kRuntime),
            program: """
                func dynamicIndex() -> u16 {
    ...
    func test_EndToEndIntegration_BlocksAreNotStackFrames_0() throws {
        ...
        XCTAssertEqual(
            a?.storage,
            .staticStorage(offset: SnapCompilerMetrics.kStaticStorageStartAddress + 0)
        )
        ...
        XCTAssertEqual(
            b?.storage,
            .staticStorage(offset: SnapCompilerMetrics.kStaticStorageStartAddress + 1)
        )
        ...
        XCTAssertEqual(
            c?.storage,
            .staticStorage(offset: SnapCompilerMetrics.kStaticStorageStartAddress + 2)
        )
    ...
    func test_EndToEndIntegration_DeclareVariableWithExplicitType_CannotConvertU16ToBool() {
        ...
        expectError(result) { error in
            ...
            XCTAssertEqual(
                error.message,
                "cannot assign value of type `integer constant 65535' to type `const bool'"
            )
        }
    }
    ...
    func test_EndToEndIntegration_FailToAssignScalarToArray() {
        ...
        expectError(result) { error in
            ...
            XCTAssertEqual(
                error.message,
                "cannot assign value of type `integer constant 1' to type `[_]const u8'"
            )
        }
    }
    ...
    func test_EndToEndIntegration_CannotAddArrayToInteger() {
        ...
        expectError(result) { error in
            ...
            XCTAssertEqual(
                error.message,
                "binary operator `+' cannot be applied to operands of types `integer constant 1' and `[3]const u8'"
            )
        }
    }
    ...
    func test_EndToEndIntegration_ArrayOfIntegerConstantsConvertedToArrayOfU16OnInitialAssignment()
        ...
        XCTAssertEqual(
            debugger.loadSymbolArrayOfU16(10, "arr"),
    ...
    func test_EndToEndIntegration_ReassignArrayContentsWithArrayIdentifier() throws {
        ...
        XCTAssertEqual(
            debugger.loadSymbolArrayOfU16(10, "a"),
            [100, 101, 102, 103, 104, 105, 106, 107, 108, 109]
        )
    }
    ...
    func test_EndToEndIntegration_CannotMakeMutatingPointerFromConstant_2() {
        ...
        expectError(result) { error in
            XCTAssertEqual(
                error.message,
                "cannot convert value of type `*const Foo' to expected argument type `*Foo' in call to `doTheThing'"
            )
        }
    }
    ...
    func testBugWhereCompilerPermitsImplicitConversionFromUnionToMemberType() {
        ...
        expectError(result) { error in
            ...
            XCTAssertEqual(
                error.message,
                "cannot implicitly convert a union type `u8 | bool' to `u8'; use an explicit conversion instead"
            )
        }
    }
