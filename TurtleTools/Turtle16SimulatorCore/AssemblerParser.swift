...
public class AssemblerParser: Parser {
    ...
    func consumeSingleParameter() throws -> Parameter {
        ...
        if let tokenNumber = accept(TokenNumber.self) as? TokenNumber {
            let paramNumber = ParameterNumber(sourceAnchor: tokenNumber.sourceAnchor, value: tokenNumber.literal)
            if let _ = accept(TokenParenLeft.self) {
                ...
                let rightParen = try expect(type: TokenParenRight.self, error: CompilerError(sourceAnchor: peek()?.sourceAnchor, message: "expected `)'")) as! TokenParenRight
                let paramIdentifier = ParameterIdentifier(sourceAnchor: identifier.sourceAnchor, value: identifier.lexeme)
                let sourceAnchor = tokenNumber.sourceAnchor?.union(rightParen.sourceAnchor)
                param = ParameterAddress(sourceAnchor: sourceAnchor,
                                         offset: paramNumber,
    ...
    func operandTypeMismatchError(sourceAnchor: SourceAnchor?) -> Error {
        return CompilerError(sourceAnchor: sourceAnchor, message: "operand type mismatch")
    }
    
    func extraneousCommaError(sourceAnchor: SourceAnchor?) -> Error {
        return CompilerError(sourceAnchor: sourceAnchor, message: "extraneous comma")
    }
}
