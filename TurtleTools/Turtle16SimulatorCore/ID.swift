...
public class ID: NSObject {
    ...
    public func step(input: Input) -> Output {
        if input.ins == 0b0110100011100001 {
            print("here")
        }
    ...
    fileprivate func generateHazardControlSignals(input: Input) -> HazardControlSignals {
        // TODO: Rewrite this method in a style which can be closely transcribed into the GAL asm HDL language.
        
        if input.j & 1 == 0 {
            return HazardControlSignals(flush: 1,
                                        stall: 0,
                                        fwd_a: 0,
                                        fwd_ex_to_a: 1,
                                        fwd_mem_to_a: 1,
        ...
        let resultStall = HazardControlSignals(flush: 1,
                                               stall: 1,
                                               fwd_a: 0,
                                               fwd_ex_to_a: 1,
                                               fwd_mem_to_a: 1,
        ...
        let selC_EX = (input.ins_EX >> 8) & 0b111
        let wben_EX = (input.ctl_EX >> 20) & 1
        let wben_MEM = (input.ctl_MEM >> 20) & 1
        
        enum RAWResolution {
            case NoForwardingRequired, MustStall, Y_EX, Y_MEM
        }
        
        let rawResolutionA: RAWResolution
        let rawResolutionB: RAWResolution
        
        // Determine whether this is a RAW hazard involving A where the result we need is in EX.
        if selA == selC_EX && wben_EX == 0 {
            let writeBackSrc_EX = (input.ctl_EX >> 17) & 1
            if writeBackSrc_EX == 0 {
                // If the result we need is in Y_EX then we can forward it.
                rawResolutionA = .Y_EX
            } else {
                // If the result we need is in storeOP_EX then we have to stall.
                rawResolutionA = .MustStall
            }
        }
        // Determine whether this is a RAW hazard involving A where the result we need is in MEM.
        else if selA == input.selC_MEM && wben_MEM == 0 {
            let writeBackSrc_MEM = (input.ctl_MEM >> 17) & 1
            if writeBackSrc_MEM == 0 {
                // If the result we need is in Y_MEM then we can forward it.
                rawResolutionA = .Y_MEM
            } else {
                // If the result we need is in storeOP_MEM then we have to stall.
                rawResolutionA = .MustStall
            }
        }
        else {
            rawResolutionA = .NoForwardingRequired
        }
        
        // Determine whether this is a RAW hazard involving B where the result we need is in EX.
        if selB == selC_EX && wben_EX == 0 {
            let writeBackSrc_EX = (input.ctl_EX >> 17) & 1
            if writeBackSrc_EX == 0 {
                // If the result we need is in Y_EX then we can forward it.
                rawResolutionB = .Y_EX
                ...
                // If the result we need is in storeOP_EX then we have to stall.
                rawResolutionB = .MustStall
            }
        }
        // Determine whether this is a RAW hazard involving B where the result we need is in MEM.
        else if selB == input.selC_MEM && wben_MEM == 0 {
            let writeBackSrc_MEM = (input.ctl_MEM >> 17) & 1
            if writeBackSrc_MEM == 0 {
                // If the result we need is in Y_MEM then we can forward it.
                rawResolutionB = .Y_MEM
            } else {
                // If the result we need is in storeOP_MEM then we have to stall.
                rawResolutionB = .MustStall
            }
            ...
            rawResolutionB = .NoForwardingRequired
        }
        
        return HazardControlSignals(flush: (rawResolutionA == .MustStall || rawResolutionB == .MustStall) ? 1 : 0,
                                    stall: (rawResolutionA == .MustStall || rawResolutionB == .MustStall) ? 1 : 0,
                                    fwd_a: (rawResolutionA == .NoForwardingRequired || rawResolutionA == .MustStall) ? 0 : 1,
                                    fwd_ex_to_a: (rawResolutionA == .Y_EX) ? 0 : 1,
                                    fwd_mem_to_a: (rawResolutionA == .Y_MEM) ? 0 : 1,
                                    fwd_b: (rawResolutionB == .NoForwardingRequired || rawResolutionB == .MustStall) ? 0 : 1,
                                    fwd_ex_to_b: (rawResolutionB == .Y_EX) ? 0 : 1,
                                    fwd_mem_to_b: (rawResolutionB == .Y_MEM) ? 0 : 1)
    }
