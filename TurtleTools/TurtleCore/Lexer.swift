...
open class Lexer {
    ...
    public func match(pattern: String) -> SourceAnchor? {
        match(rule: Rule(pattern: pattern, emit: { _ in nil }))
    }
    ...
    public func match(rule: Rule) -> SourceAnchor? {
        ...
        guard
            let match = regex.firstMatch(
                in: string,
                options: [],
                range: NSRange(position..., in: string)
            )
        else {
            return nil
        }
    ...
    public func scanTokens() {
        ...
        while !isAtEnd {
            do {
                ...
                errors.append(
                    CompilerError(
                        sourceAnchor: tokens.last?.sourceAnchor,
        ...
        let eofAnchor = SourceAnchor(
            range: string.endIndex..<string.endIndex,
    ...
    public func scanToken() throws {
        ...
        throw unexpectedCharacterError(
            sourceAnchor: SourceAnchor(range: peekRange()!, lineMapper: lineMapper)
        )
    }
    ...
    func unexpectedCharacterError(sourceAnchor: SourceAnchor) -> CompilerError {
        CompilerError(
            ...
            message: "unexpected character: `\(sourceAnchor.text)'"
        )
    }
