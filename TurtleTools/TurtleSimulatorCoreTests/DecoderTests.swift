...
final class ProgrammableLogicDecoderTests: XCTestCase {
    ...
    fileprivate func doesSignalMatchReference(
        _ name: String,
        _ signal: Int,
        _ actual: [UInt],
        _ expected: [UInt]
    ...
    func testSignals() throws {
        let pld = ProgrammableLogicDecoder()
        var actual = [UInt](repeating: 0, count: pld.count)
        for i in 0..<pld.count {
            actual[i] = pld.decode(i)
        }

        var expected = [UInt](repeating: 0, count: pld.count)
        let reference = OpcodeDecoderROM()
        reference.opcodeDecodeROM = DecoderGenerator().generate()
        for i in 0..<pld.count {
        ...
        XCTAssertTrue(
            doesSignalMatchReference("SelStoreOpA", DecoderGenerator.SelStoreOpA, actual, expected)
        )
        XCTAssertTrue(
            doesSignalMatchReference("SelStoreOpB", DecoderGenerator.SelStoreOpB, actual, expected)
        )
        XCTAssertTrue(
            doesSignalMatchReference("SelRightOpA", DecoderGenerator.SelRightOpA, actual, expected)
        )
        XCTAssertTrue(
            doesSignalMatchReference("SelRightOpB", DecoderGenerator.SelRightOpB, actual, expected)
        )
        ...
        XCTAssertTrue(
            doesSignalMatchReference("MemLoad", DecoderGenerator.MemLoad, actual, expected)
        )
        XCTAssertTrue(
            doesSignalMatchReference("MemStore", DecoderGenerator.MemStore, actual, expected)
        )
        XCTAssertTrue(
            doesSignalMatchReference(
                "AssertStoreOp",
        ...
        XCTAssertTrue(
            doesSignalMatchReference(
                "WriteBackSrcFlag",
                DecoderGenerator.WriteBackSrcFlag,
                actual,
                expected
            )
        )
